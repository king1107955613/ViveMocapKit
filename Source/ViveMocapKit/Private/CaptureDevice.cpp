	...
	if (bIsInitialized && IsValid(BodyMesh->SkeletalMesh))
	{
		...
		for (const auto& socketbone : SocketsMap)
		{
			...
			if (!sk.bIsLocator)
			{
				...
				if (bDebugLog)
				{
					FString attachbone = SkeletonBonesMap.Contains(sk.AttachmentBone) ? SkeletonBonesMap[sk.AttachmentBone].ToString() : "unknown";
					UE_LOG(LogCaptureDevice, Log, TEXT("Add Socket Bone [%s] attached to: %s with offset %s"), *socketbone.Key.ToString(), *attachbone, *sk.OffsetToRootParentBone.ToString());
				}
			}
			else
			{
...
void UCaptureDevice::MeasureVerticalAdjustment(float FootHeight)
{
	MeasureFootHeight = FootHeight;
	bMeasureVerticalAdjustment = true;
}
...
void UCaptureDevice::CalibrateBones(bool bPredictOnly)
{
	...
	*/
	...
	for (int32 id : DeviceIds)
	{
		...
		GetVRTrackerPosition(id, loc, rot);		// tracker relative location and rotation
		T2HMD = FTransform(rot, loc);			// tracker relative transform
		T2World = T2HMD * ActorTransform;		// tracker world transform
		T2HMD = MakeRelativeTransform(T2World, CameraTransform);	// tracker transform relative to camera
	...
	*/
	...
	for (auto& tracker : TrackersData)
	{
		...
		{
			const FName BoneName = SkeletonBonesMap[tracker.Value.Bone];
			FTransform wTracker;
			// get bone transform
			FTransform wBone = BodyMesh->GetSocketTransform(BoneName, ERelativeTransformSpace::RTS_Component) * BodyMesh->GetComponentTransform();
			...
			// test
			wBone = FTransform::Identity;
			wBone.SetTranslation(BodyMesh->GetBoneLocation(BoneName));
			wBone.SetRotation(BodyMesh->GetBoneQuaternion(BoneName));
			// end test
...
void UCaptureDevice::GetSkeletalMeshPose(FPoseSnapshot& ReturnValue, bool bEnableScaling)
{
	...
	if (bHasRootBone)
	{
		while (NextBoneName != PelvisBoneName)
		{
			...
			// check socket bone for skipped
			if (const FSocketInfo* SocketBone =  SocketsInfo.Find(NextBoneName))
			{
				if (SocketBone->bIsLocator)
				{
					...
					ReturnValue.LocalTransforms[StartBoneIndex] = tr_bone.GetRelativeTransform(tr_parent);
				}
				else
				{
					AttachedSocketBones.Add(StartBoneIndex);
	...
	int32 RightFootIndex = INDEX_NONE;
	int32 LeftFootIndex = INDEX_NONE;
	...
	for (int32 ComponentSpaceIdx = StartBoneIndex; ComponentSpaceIdx < NumSpaceBases; ++ComponentSpaceIdx)
	{
		...
		if (BoneName == SkeletonBonesMap[EHumanoidBone::FootRight])
		{
			RightFootIndex = ComponentSpaceIdx;
		}
		else if (BoneName == SkeletonBonesMap[EHumanoidBone::FootLeft])
		{
			LeftFootIndex = ComponentSpaceIdx;
		}
		...
		/* update normal bone */
		else if (bBoneHasEvaluated && bIsCapturing)
		{
	...
	if (!bEnableScaling || bMeasureVerticalAdjustment)
	{
		for (int32 ComponentSpaceIdx = StartBoneIndex; ComponentSpaceIdx < NumSpaceBases; ++ComponentSpaceIdx)
		{
			...
			if (ComponentSpaceIdx == PelvisBoneIndex)
			{
				ReturnValue.LocalTransforms[ComponentSpaceIdx].AddToTranslation(FVector::UpVector * RuntimeVerticalOffset);
	...
	if (bMeasureVerticalAdjustment)
	{
		bMeasureVerticalAdjustment = false;
		const FVector FootRightLoc = iRestoreSkeletonBoneTransform(ReturnValue, RefSkeleton, RightFootIndex).GetTranslation();
		const FVector FootLeftLoc = iRestoreSkeletonBoneTransform(ReturnValue, RefSkeleton, LeftFootIndex).GetTranslation();

		const FVector FeetGround = (FootRightLoc + FootLeftLoc) * 0.5f - RuntimeVerticalOffset;
		RuntimeVerticalOffset = (FeetGround.Z - MeasureFootHeight) * -1.f;
	}
	...
	for (auto ComponentSpaceIdx : AttachedSocketBones)
	{
		...
		if (bDebugLog)
		{
			UE_LOG(LogCaptureDevice, Log, TEXT("Update socket %d = %s"), ComponentSpaceIdx, *BoneName.ToString());
		}
		...
		if (bDebugLog)
		{
			UE_LOG(LogCaptureDevice, Log, TEXT("tr_bone = %s"), *tr_bone.ToString());
		}
	...
	if (bMultiMeshUpdate)
	{
		LastPoseSnapshot = ReturnValue;
	}
}

void UCaptureDevice::GetLastSkeletalMeshPose(UPARAM(ref) FPoseSnapshot& ReturnValue, bool bUnscaleSnapshot)
{
	ReturnValue = LastPoseSnapshot;

	if (bUnscaleSnapshot && LastPoseSnapshot.bIsValid)
	{
		const TArray<FTransform>& RefPoseSpaceBaseTMs = BodyMesh->SkeletalMesh->RefSkeleton.GetRefBonePose();
		const int32 NumSpaceBases = BodyMesh->GetComponentSpaceTransforms().Num();
